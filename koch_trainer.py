...
class KochTrainerAudioGen:
    def __init__( self, message, effective_speed=19, character_speed=22, hertz=600, bandwidth=200, end_char_delay=0.0017 ):
        ...
        self.end_char_delay = end_char_delay
    ...
    def emit_audio( self ):
        try:
            stream = audiogen_p3.sampler.play( itertools.chain( self._audio, audiogen_p3.silence(self.end_char_delay) ), blocking=True )
    ...
    def inter_letter( self ):
        for sample in audiogen_p3.silence( self._inter_letter * 1.6):
            yield sample
